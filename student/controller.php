...
switch ($action) {
	...
	function doInsert(){
		...
		if ($_POST['StudentID'] == "" OR $_POST['Firstname'] == "" OR $_POST['Lastname'] == ""
			...
			OR $_POST['ContactNo'] == "" OR $_POST['faceID'] == ""OR $_POST['guardianEmail'] == "" ) {
			$messageStats = false;
			message("All fields are required!","error");
			redirect('index.php?view=add');
		}else{	
			if (strlen($_POST['StudentID'])< 8){
				message("Invalid Student ID. It should have 8 digits.", "error");
				redirect("index.php?view=add");
			}else{
				$birthdate =  $_POST['year'].'-'.$_POST['month'].'-'.$_POST['day'];
				$age = date_diff(date_create($birthdate),date_create('today'))->y;
				if ($age < 15){
				message("Invalid age. Student should be ateleast 16 years old", "error");
				redirect("index.php?view=add");
				}else{
					// error message 
					...
					if ($maxrow > 0) { 
						...
						$stud->guardianEmail	= $_POST['guardianEmail'];
						$stud->create();
	...
	if(isset($_POST['save'])){
		...
		OR $_POST['ContactNo'] == "") {
		  ...
		  // echo  $_POST['optionsRadios']; 
				...
				$stud->ContactNo		= $_POST['ContactNo'];
				$stud->YearLevel		= $_POST['YearLevel'];
				$stud->guardianEmail	= $_POST['guardianEmails'];
